...
function WelcomePage(props) {
  ...
  useEffect(() => {
    ...
    function flashWinningCells(on) {
        if (!Array.isArray(win.winningCells)) return;
  ...
  useEffect(() => {
    ...
    function createWinState(start, winType) {
        ...
        while (true) {
          ...
          if (current !== win.winner) break;
          win.winningCells.push(pos);
          if (winType === winTypes.vertical) {
            pos.row++;
          } else if (winType === winTypes.horizontal) {
            pos.column++;
          ...
          if (!board[getIndex(pos.row, pos.column)]) break;
        }
      
        if (win.winningCells.length < 4) {
          return null;
        }
    ...
    function isVerticalWin() {
      ...
      for (let r = 0; r <= rows - 4; r++) {
        for (let c = 0; c < columns; c++) {
          ...
          if (
            cell === board[getIndex(r + 1, c)] &&
            cell === board[getIndex(r + 2, c)] &&
            cell === board[getIndex(r + 3, c)]
          ) {
            setWin({
              winner: cell,
              winningCells: [
    ...
    setWin(isWin());
  }, [board, dropping, win, currentPlayer]);
